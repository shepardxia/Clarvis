...
class MemoryConfig(BaseModel):
    """Configuration for dual memory system (Graphiti + memU)."""

    enabled: bool = Field(default=False, description="Enable dual memory system (Graphiti + memU)")
    data_dir: str = Field(default="~/.clarvis/memory", description="Root directory for memory storage")
    model: str = Field(default="claude-sonnet-4-6", description="LLM model for memory extraction (Graphiti + memU)")
    ...
    datasets: dict[str, DatasetConfig] = Field(
        ...
        description="Dataset definitions with visibility scoping.",
    )
...
class WidgetConfig(BaseModel):
    ...
    def save(self, path: Path = CONFIG_PATH) -> None:
        json_save_atomic(path, self.model_dump())
