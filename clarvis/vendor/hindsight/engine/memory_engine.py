...
from .retain.types import FactInput, RetainContentDict
from .search.reranking import CrossEncoderReranker
from .search.tags import TagsMatch
from .task_backend import BrokerTaskBackend, TaskBackend
...
class MemoryEngine(MemoryEngineInterface):
    ...
    async def retain_direct_async(
        ...
        *,
        ...
        """Store pre-structured facts (Level 2 entry point).

        Bypasses LLM fact extraction entirely.  The caller is responsible for
        constructing well-formed ``FactInput`` objects with correct fact_type,
        entities, temporal range, confidence, and provenance.

        Hindsight handles: embedding generation, deduplication, storage,
        entity graph processing, and link creation (temporal, semantic,
        entity, causal).

        Args:
            bank_id: Unique identifier for the bank
            facts: List of pre-structured FactInput objects
            request_context: Request context for authentication.

        Returns:
            List of created fact IDs (UUIDs as strings).
        """
        if not facts:
            return []
        ...
        async with self._put_semaphore:
            ...
            with create_operation_span("retain_direct", bank_id):
                return await orchestrator.retain_direct(
                    pool=pool,
                    embeddings_model=self.embeddings,
                    entity_resolver=self.entity_resolver,
                    format_date_fn=self._format_readable_date,
    ...
    # =========================================================================
    # Level 2 Consolidation Entry Points
    #
    # These methods split the consolidation pipeline so an external agent
    # (e.g. Clarvis) can make the LLM decisions instead of Hindsight's
    # internal LLM.  They expose the non-LLM bookkeeping as public API.
    # =========================================================================

    async def get_unconsolidated_async(
        self,
        bank_id: str,
        *,
        limit: int = 100,
        ...
        Returns facts with ``consolidated_at IS NULL`` and
        ``fact_type IN ('experience', 'world')``, ordered by creation time.
        ...
        Returns:
            Dict with ``facts`` (list of dicts) and ``total_count`` (int).
        """
        await self._authenticate_tenant(request_context)
        pool = await self._get_pool()
    ...
    async def get_related_observations_async(
        self,
        bank_id: str,
        fact_texts: list[str],
        ...
        *,
        ...
        """For each ``(text, tags)`` pair, find overlapping observations via recall.

        Mirrors the parallel-recall + union logic in
        ``_process_memory_batch`` (consolidator.py) without calling the LLM.

        Args:
            bank_id: Bank identifier.
            fact_texts: List of fact text strings.
            fact_tags: List of tag lists, one per fact text.
            request_context: Request context for authentication.

        Returns:
            Dict with ``observations`` (list of model_dump'd observations),
            ``source_facts`` (dict), and ``per_fact_obs_ids``
            (mapping of fact text to list of observation ID strings).
        """
        import asyncio

        await self._authenticate_tenant(request_context)

        from .consolidation.consolidator import _find_related_observations

        # Parallel recalls — one per fact text (same pattern as _process_memory_batch)
        recall_tasks = [
        ...
        # Build per-fact observation ID sets (keyed by fact text)
        ...
        # Union all observations (deduped by id) — same logic as _process_memory_batch
    ...
    async def apply_consolidation_decisions_async(
        self,
        bank_id: str,
        decisions: list,
        ...
        *,
        ...
        """Apply agent-driven consolidation decisions and mark facts as consolidated.

        Reuses the existing action executors from ``consolidator.py`` and
        preserves all security validation (observation ID checking).

        Args:
            bank_id: Bank identifier.
            decisions: List of ``ConsolidationDecision`` objects.
            fact_ids_to_mark: Fact IDs to mark as consolidated after execution.
            related_observations: Observation objects from
                ``get_related_observations_async`` (needed for update/delete
                security validation).
            request_context: Request context for authentication.

        Returns:
            Dict with counts: ``created``, ``updated``, ``deleted``, ``marked``.
        """
        import uuid as uuid_module
        ...
        # Build a set of valid observation IDs from the related observations
        # for security validation of update/delete operations.
            ...
            for d in decisions:
                if d.action == "create":
                    ...
                    if not d.observation_id:
                        logger.warning("[CONSOLIDATION L2] Rejected update — no observation_id provided")
                        continue
                    # Security: observation must be in the related set
                    if d.observation_id not in valid_obs_ids:
                        logger.warning(
                            f"[CONSOLIDATION L2] Rejected update — observation {d.observation_id} "
                            f"not in related observations set"
                        )
                        continue
                    source_uuids = [uuid_module.UUID(fid) for fid in d.source_fact_ids]
                    ...
                    if not d.observation_id:
                        logger.warning("[CONSOLIDATION L2] Rejected delete — no observation_id provided")
                        continue
                    # Security: observation must be in the related set
                    if d.observation_id not in valid_obs_ids:
                        logger.warning(
                            f"[CONSOLIDATION L2] Rejected delete — observation {d.observation_id} "
                            f"not in related observations set"
                        )
                        continue
    ...
    async def list_models_needing_refresh_async(
        self,
        bank_id: str,
        ...
        *,
        ...
        Extracts the tag-filtered query from ``_trigger_mental_model_refreshes``
        but returns the list instead of triggering refresh tasks.

        Args:
            bank_id: Bank identifier.
            consolidated_tags: Tags from the consolidated facts.  When provided,
                returns models with overlapping tags OR untagged models.
                When ``None``, returns only untagged models.
            request_context: Request context for authentication.

        Returns:
            List of dicts with ``id``, ``name``, ``tags``, ``source_query``,
            ``content``.  Does NOT trigger any refresh — caller decides.
        """
        await self._authenticate_tenant(request_context)
        pool = await self._get_pool()
        ...
        async with acquire_with_retry(pool) as conn:
            if consolidated_tags:
                # Tagged facts consolidated — return models with overlapping tags OR untagged
                rows = await conn.fetch(
                    f"""
                ...
                # Untagged facts consolidated — only return untagged models
                rows = await conn.fetch(
                    f"""
                    SELECT id, name, tags, source_query, content
                    FROM {fq_table("mental_models")}
    ...
    # ==================== Reflect Tools (Level 2 Public API) ====================

    async def search_mental_models_async(
        self,
        bank_id: str,
        query: str,
        *,
        ...
        """
        Search mental models by semantic similarity (non-LLM).

        Generates a query embedding and searches the mental_models table via pgvector.
        Includes staleness info based on pending consolidation count.

        Args:
            bank_id: Bank identifier
            query: Search query text
            max_results: Maximum number of mental models to return (default 5)
            tags: Optional tags to filter mental models
            tags_match: How to match tags - "any" (OR), "all" (AND)
            request_context: Request context for authentication

        Returns:
            Dict with matching mental models including content, relevance, and staleness info
        """
        await self._authenticate_tenant(request_context)
        pool = await self._get_pool()

        # Generate query embedding
        embeddings = await embedding_utils.generate_embeddings_batch(self.embeddings, [query])
        query_embedding = embeddings[0]

        # Get pending consolidation count for staleness info
        async with acquire_with_retry(pool) as conn:
            pending_consolidation = await conn.fetchval(
    ...
    async def search_observations_async(
        ...
        *,
        ...
        """
        Search consolidated observations using TEMPR retrieval (non-LLM).

        Returns observations with source facts and staleness info based on
        ...
        Returns:
            Dict with matching observations, source facts, and freshness info
        """
        await self._authenticate_tenant(request_context)
        pool = await self._get_pool()
    ...
    async def recall_facts_async(
        ...
        *,
        ...
        """
        Recall raw facts (experience + world) using TEMPR retrieval (non-LLM).

        Thin wrapper around recall_async that fixes fact_type to raw facts
        and includes chunk context. Used by the external agent for ground-truth
        verification during reflect and check-in workflows.

        Args:
            bank_id: Bank identifier
            query: Search query text
            max_tokens: Maximum tokens for results (default 5000)
            tags: Optional tags to filter facts
            tags_match: How to match tags - "any" (OR), "all" (AND)
            request_context: Request context for authentication

        Returns:
            Dict with matching facts and their chunk context
        """
    ...
    async def expand_async(
        ...
        *,
        ...
        """
        Expand memories to get chunk or document context (non-LLM).

        Traverses the memory -> chunk -> document hierarchy for the given
        memory IDs. Used by the external agent to get full context around
        specific facts.

        Args:
            bank_id: Bank identifier
            memory_ids: List of memory unit IDs to expand
            depth: Expansion depth - "chunk" or "document" (default "chunk")
            request_context: Request context for authentication

        Returns:
            Dict with expanded results containing memory, chunk, and optionally document data
        """
        await self._authenticate_tenant(request_context)
        pool = await self._get_pool()
