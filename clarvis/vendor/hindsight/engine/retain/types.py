...
@dataclass
class FactInput:
    """Structured fact from an external agent (bypasses LLM extraction).

    Used by retain_direct() to store pre-structured facts without invoking
    LLM fact extraction. The agent is responsible for fact_type classification,
    entity identification, temporal parsing, and confidence scoring.
    """
...
class ConsolidationDecision:
    """A consolidation action decided by an external agent (not by Hindsight's LLM).

    Used by apply_consolidation_decisions_async to execute agent-driven
    consolidation without involving Hindsight's internal LLM.
    """

    action: str  # "create" | "update" | "delete"
    text: str  # observation content (new or updated)
    source_fact_ids: list[str]  # fact IDs that support this decision
    observation_id: str | None = None  # target observation (for update/delete)
