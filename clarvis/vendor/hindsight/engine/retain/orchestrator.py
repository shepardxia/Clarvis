...
from .types import EntityRef, ExtractedFact, FactInput, ProcessedFact, RetainContent, RetainContentDict
...
async def retain_direct(
    ...
    """Store pre-structured facts, bypassing LLM extraction.

    This is the Level 2 entry point: the caller (Clarvis) has already done
    fact extraction, entity identification, temporal parsing, and confidence
    scoring.  We enter the retain pipeline at Step 2 (embeddings) and reuse
    every non-LLM sub-function that ``retain_batch`` uses.
    ...
    Args:
        ...
        config: Resolved HindsightConfig for this bank
        schema: Optional database schema (for multi-tenant isolation)
    ...
    log_buffer = []
    log_buffer.append(f"{'=' * 60}")
    log_buffer.append(f"RETAIN_DIRECT START: {bank_id}")
    log_buffer.append(f"Batch size: {len(facts)} pre-structured facts")
    log_buffer.append(f"{'=' * 60}")

    if not facts:
        logger.info("RETAIN_DIRECT COMPLETE: 0 facts (empty input)")
        return []

    # -- Step 1 (SKIPPED): No LLM fact extraction --------------------------

    # -- Convert FactInput -> ProcessedFact --------------------------------
    # We need mentioned_at for each fact (the current timestamp).
    now = utcnow()
    ...
    for fi in facts:
        ...
        # Placeholder embedding — will be replaced after generation below
        pf = ProcessedFact(
    ...
    # -- Step 2: Augment texts and generate embeddings ---------------------
    # augment_texts_with_dates accesses .occurred_start, .mentioned_at,
    # .fact_text — ProcessedFact has all three, so duck-typing works.
    step_start = time.time()
    augmented_texts = embedding_processing.augment_texts_with_dates(processed_facts, format_date_fn)
    embeddings = await embedding_processing.generate_embeddings_batch(embeddings_model, augmented_texts)
    log_buffer.append(f"[2] Generate embeddings: {len(embeddings)} embeddings in {time.time() - step_start:.3f}s")

    # Patch the real embeddings into the ProcessedFact objects
    for pf, emb in zip(processed_facts, embeddings):
        pf.embedding = emb

    # -- Step 3: Document tracking + dedup + insert + entities + links -----
    async with acquire_with_retry(pool) as conn:
        async with conn.transaction():
            # Ensure bank exists
            await fact_storage.ensure_bank_exists(conn, bank_id)

            # Handle document tracking for facts that carry a document_id
            step_start = time.time()
            ...
            for pf in processed_facts:
                if pf.document_id and pf.document_id not in doc_ids_seen:
                    ...
                    # Build combined content for all facts sharing this document_id
            ...
            if doc_ids_seen:
                log_buffer.append(
                    f"[2.5] Document tracking: {len(doc_ids_seen)} documents in {time.time() - step_start:.3f}s"
                )
            ...
            if not non_duplicate_facts:
                logger.info("RETAIN_DIRECT COMPLETE: 0 facts (all duplicates)")
                return []
        ...
        total_time = time.time() - start_time
        log_buffer.append(f"{'=' * 60}")
        log_buffer.append(f"RETAIN_DIRECT COMPLETE: {len(unit_ids)} units in {total_time:.3f}s")
        log_buffer.append(f"{'=' * 60}")
